...
var FormGroupAsync = function () {
  ...
  _createClass(FormGroupAsync, [{
    ...
    key: 'addGroup',
    value: function addGroup(groupKeyToAdd) {
      if (typeof groupKeyToAdd !== 'string') {
        throw new Error(groupKeyToAdd + ' - Group name is expected to be a string');
      }

      this.groups = [].concat(_toConsumableArray(this.groups), [groupKeyToAdd]);
      this.generatePromises();
    }
  }, {
    key: 'removeGroup',
    value: function removeGroup(groupKeyToRemove, isForceRemove) {
      if (typeof groupKeyToRemove !== 'string') {
        throw new Error(groupKeyToRemove + ' - Group name is expected to be a string');
      }

      var hasGroupKey = this.groups.filter(function (group) {
        return group === groupKeyToRemove;
      })[0];
